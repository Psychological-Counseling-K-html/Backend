...
from ragas.metrics import (
    embedding_f1,
    answer_relevance,
    contextual_factuality,
    retrieval_factuality
)
...
class ChatService:
    ...
    async def create_chat(self, db: Session, create_chat_input: CreateChatInput, user_id: str) -> Chat:
        ...
        while attempts < max_retries:
            ...
            print(f"Attempt {attempts+1}: Generated answer: {response}")

            reference_answer = " ".join(
                [doc.page_content for doc in rag_responses])

            # 평가를 위한 DataFrame 구성
            data_dict = {
                "query": [create_chat_input.ask],
                "retrieved_docs": [[doc.page_content for doc in rag_responses]],
            ...
            # SentenceTransformerEmbeddings 모델을 사용하여 평가
            results = evaluation.run(ragas.embedding_model)

            # retrieval_factuality 메트릭을 기준으로 평가
            ragas_score = results.get("retrieval_factuality", 0)
            print(f"RAGAS score on attempt {attempts+1}: {ragas_score:.4f}")
            ...
            # 그렇지 않으면 재시도
            attempts += 1
            best_answer = response  # 최종 시도 응답을 fallback으로 사용
